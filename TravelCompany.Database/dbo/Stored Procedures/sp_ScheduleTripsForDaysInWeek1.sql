...
CREATE procedure [dbo].[sp_ScheduleTripsForDaysInWeek1]
				@RouteId int,
				 @Time time,
...
from Dates d 
--where (exists (select 1 from @Days d where DATEPART(WEEKDAY, d.Date )= d.Day))
where DATEPART(WEEKDAY, d.Date ) in (select *from @Days)
and (not exists ( select 1 from ExistedTripsDates e where e.Date=d.Date))
