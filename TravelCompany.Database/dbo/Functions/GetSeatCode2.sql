...
--set @SeatCode= dbo.GetSeatCode2(3, 7 )
--print @SeatCode

CREATE function [dbo].[GetSeatCode2](@StaionAOrder int , @StaionBOrder int )
returns Bigint
as 
begin
...
while(@StaionAOrder<@StaionBOrder)
begin

set @SeatCode=@SeatCode |1* power(2,@StaionAOrder-1);
-- 1 * power (2, @CurrentStationOrder) equils 1 << @CurrentStationOrder in c# (shift to lift)

set @StaionAOrder=@StaionAOrder+1;
end
